...
class GetRecommendedMoviesByYearUseCase @Inject constructor(
    private val repository: MovieRepositoryContract
) {
    operator fun invoke(year: Int) : Flow<Resource<List<Movie>>> =
        repository.getRecommendedMoviesByYear(year)
}
